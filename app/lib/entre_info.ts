export const entreInfo = {
  empresa: {
    nome: "Entre",
    descricao: "Empresa focada em melhorar as operações dos clientes através de ajustes práticos, automação e implementação de IA",
    diferencial: "Diferente de consultorias tradicionais, integramos diretamente na rotina do cliente",
    proposta_valor: "Transformamos operações empresariais com foco em resultados mensuráveis e implementação prática",
    fundacao: "Empresa moderna com abordagem inovadora em consultoria operacional"
  },
  
  servicos: {
    diagnostico_operacional: {
      nome: "Diagnóstico Operacional",
      descricao: "Análise completa dos processos atuais da empresa para identificar oportunidades de melhoria",
      diferencial: "Diagnóstico preliminar gratuito",
      entregaveis: ["Mapeamento de processos", "Identificação de gargalos", "Relatório de oportunidades"]
    },
    redesenho_sistemas: {
      nome: "Redesenho de Sistemas e Processos",
      descricao: "Reestruturação completa de processos operacionais para máxima eficiência",
      foco: "Simplificação e otimização de fluxos de trabalho"
    },
    automacao_fluxos: {
      nome: "Implementação de Fluxos e Automações",
      descricao: "Automatização de processos repetitivos e implementação de fluxos inteligentes",
      ferramentas: ["ClickUp", "Notion", "Make", "Zapier"],
      beneficios: ["Redução de trabalho manual", "Eliminação de erros", "Ganho de velocidade"]
    },
    gestao_projetos: {
      nome: "Gestão Estratégica de Projetos",
      descricao: "Acompanhamento e gestão de projetos de transformação operacional",
      abordagem: "Metodologias ágeis adaptadas à realidade de cada cliente"
    },
    treinamento_ia: {
      nome: "Treinamento em IA e Operações",
      descricao: "Capacitação de equipes para uso de inteligência artificial aplicada às operações",
      formato: "Treinamentos práticos e personalizados"
    }
  },
  
  resultados: {
    metricas_sucesso: [
      "Redução de tempo em processos operacionais",
      "Aumento da capacidade operacional",
      "Diminuição de erros e retrabalho",
      "Otimização de custos operacionais"
    ],
    compromisso: "Nossos resultados são tangíveis e mensuráveis - não fazemos consultoria teórica"
  },
  
  processo_comercial: {
    etapas: [
      {
        fase: "Contato Inicial",
        descricao: "Primeiro contato para entender as necessidades"
      },
      {
        fase: "Conversa Inicial",
        descricao: "Análise da situação atual"
      },
      {
        fase: "Proposta",
        descricao: "Apresentação de como podemos ajudar"
      },
      {
        fase: "Implementação",
        descricao: "Execução do projeto com acompanhamento contínuo"
      }
    ]
  },
  
  diferenciais: [
    "Integração direta na rotina do cliente",
    "Equipe sênior com experiência prática em operações",
    "Foco em projetos com potencial real de transformação",
    "Metodologia prática, não teórica",
    "Uso de ferramentas modernas de automação e IA"
  ],
  
  atendimento: {
    modalidade: "Remoto para todo o Brasil",
    presencial: "Disponível mediante acordo",
    canais: {
      email: "contato@entre.com.br",
      whatsapp: "Disponível",
      telefone: "Disponível"
    },
    tempo_resposta: "24 horas úteis",
    horario: "Segunda a Sexta, horário comercial"
  },
  
  ferramentas: {
    gestao: ["ClickUp", "Notion"],
    automacao: ["Make", "Zapier"],
    comunicacao: ["Slack", "Microsoft Teams"],
    ia: ["ChatGPT", "Claude", "Ferramentas customizadas"]
  },
  
  publico_alvo: {
    porte: "Pequenas, médias e grandes empresas",
    segmentos: "Todos os segmentos que buscam eficiência operacional",
    perfil: "Empresas que valorizam resultados práticos e mensuráveis"
  },
  
  faq: [
    {
      pergunta: "Como começamos?",
      resposta: "Conversamos. Entendemos sua operação. Identificamos o que pode melhorar. Simples assim."
    },
    {
      pergunta: "Quanto tempo leva?",
      resposta: "Cada operação tem seu tempo. Não prometemos prazos mágicos. Fazemos direito."
    },
    {
      pergunta: "Como funciona o pagamento?",
      resposta: "Conversamos sobre isso quando fizer sentido. Cada projeto é único."
    },
    {
      pergunta: "Precisamos parar nossa operação?",
      resposta: "Não. Trabalhamos integrados à sua rotina. A transformação acontece enquanto você opera."
    },
    {
      pergunta: "Que tipo de empresa vocês atendem?",
      resposta: "Empresas que querem evoluir. Tamanho é detalhe. Vontade de melhorar é fundamental."
    },
    {
      pergunta: "Quais ferramentas usam?",
      resposta: "ClickUp, Notion, Make, Zapier e o que mais fizer sentido. Ferramenta é meio, não fim."
    },
    {
      pergunta: "E depois da implementação?",
      resposta: "Continuamos juntos enquanto fizer sentido. Sem amarras, sem drama."
    }
  ]
};
